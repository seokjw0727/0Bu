...
async def on_ready():
  await bot.change_presence(activity=discord.Activity(
      type=discord.ActivityType.listening, name="공부야"))
  os.system('cls')
  print('-------------------------')
...
@bot.tree.command(name="상태", description="네트워크 상태를 확인합니다."
                  )  # '상태' - 봇의 네트워크 상태를 embed로 대답하는 명령어입니다.
async def network_status(interaction: discord.Interaction):
  if bot.latency * 1000 >= 100:
    not_good = discord.Embed(
        title="**🔴 이상 🔴**",
        description=
        f"현재 네트워크 지연시간은 **{round(round(bot.latency, 4)*1000)}ms** 이므로, 석지우에게 관리를 요청하세요.",
    ...
    good = discord.Embed(
        title="**🟢 정상 🟢**",
        description=
        f"현재 네트워크 지연시간은 **{round(round(bot.latency, 4)*1000)}ms** 으로, 정상입니다.",
        timestamp=datetime.datetime.now(pytz.timezone('UTC')),
        color=0x00ff00)
...
@bot.tree.command(name="공지",
                  description="/공지 [내용] | ⚙️")  # '공지' - 공지 채널에 내용을 공지합니다.
...
async def notice_error(interaction: discord.Interaction, error):
  if isinstance(error, commands.MissingPermissions):
    ...
    await log_channel.send(
        f"{interaction.user.mention} 이(가)   */공지*   명령어를 사용하였으나, 관리자 권한이 없으므로 거부당했습니다."
    )

    embed = discord.Embed(title='❌거부❌',
                          description='당신은 관리자 권한이 없습니다.',
                          color=0xff0000)
    await interaction.response.send_message(embed=embed)


@bot.tree.command(name="청소",
                  description="/청소 [1 이상의 자연수] | ⚙️")  # '청소' - 채널의 메시지를 삭제합니다.
@commands.has_permissions(administrator=True)
...
async def clear_chat(interaction: discord.Interaction, amount: int):
  ...
  if amount < 1:
    embed = discord.Embed(title='*오류!*',
                          description='1 이상의 자연수를 입력해주세요.',
                          color=0xff0000)
    await interaction.response.send_message(embed=embed, ephemeral=True)
  else:
    await interaction.response.send_message("청소를 시작합니다...")
    await interaction.channel.purge(limit=amount + 1)
    embed = discord.Embed(title="🧹 **채팅 청소** 🧹",
                          description=f"__{amount}__ 개의 메시지를 청소했습니다. \n 　",
                          timestamp=datetime.datetime.now(
                              pytz.timezone('UTC')),
                          color=0x99ffff)
    embed.add_field(name="*처리자*",
...
async def clear_chat_error(interaction: discord.Interaction, error):
  if isinstance(error, commands.MissingPermissions):
    log_channel = bot.get_channel(1157241419518459974)
    await log_channel.send(
        f"{interaction.user.mention} 이(가)   */청소*   명령어를 사용하였으나, 관리자 권한이 없으므로 거부당했습니다."
    )
...
async def credit(interaction: discord.Interaction):
  embed = discord.Embed(title='🎖️ **크레딧** 🎖️',
                        description='서버 관리봇, "0Bu"입니다. \n 편하게 공부라고 불러주세요!',
                        color=0xffffff)
